...
use crate::pipeline::{Gradient, SamplerFilter, Texture, TextureFormat, TextureRegion};
use alloc::boxed::Box;
use alloc::vec::Vec;
...
impl SoftwareBackend {
    ...
    pub fn execute_command(&mut self, command: &RenderCommand) {
        match command {
            ...
            RenderCommand::DrawText { position, text, color, size } => {
                self.draw_text_bitmap(*position, text.as_str(), *color, *size);
            }
    ...
    fn draw_text_bitmap(&mut self, position: Vec2, text: &str, color: Color, size: f32) {
        let scale = ((size / 8.0).max(1.0)) as u32;
        let glyph_w = 5 * scale;
        let advance = glyph_w + scale;
        let mut cursor_x = position.x.max(self.viewport.x) as u32;
        let mut cursor_y = position.y.max(self.viewport.y) as u32;

        for ch in text.chars() {
            if ch == '\n' {
                cursor_x = position.x.max(self.viewport.x) as u32;
                cursor_y += 8 * scale;
                continue;
            }
            ...
            for (row, bits) in glyph.iter().enumerate() {
                for col in 0..5u32 {
                    ...
                    self.fill_rect(Rect::new(px as f32, py as f32, scale as f32, scale as f32), color);
                }
            }
    ...
    fn fill_rect_tinted(&mut self, rect: Rect, region: TextureRegion, color: Color) {
        let Some(texture) = self.textures.get(&region.texture_id) else {
            self.fill_rect(rect, color);
            return;
        };
        let texture = texture as *const Texture;
        // Rendering mutates the framebuffer but not the texture map entry.
        // Keeping a raw pointer here avoids cloning full-screen textures every frame.
        unsafe {
    ...
    fn fill_rect_textured(&mut self, rect: Rect, texture: &Texture, region: &TextureRegion, tint: Color) {
        if tint == Color::WHITE
            ...
            && region_is_full(region)
            && self.blit_rgb565_exact(rect, texture)
        {
            return;
        }

        if texture.format == TextureFormat::A8
            && texture.sampler.mag_filter == SamplerFilter::Nearest
            && region_is_full(region)
            && self.blit_a8_nearest(rect, texture, tint)
        {
            return;
        }
    ...
    fn blit_rgb565_exact(&mut self, rect: Rect, texture: &Texture) -> bool {
        let rect_x = rect.x as i32;
        let rect_y = rect.y as i32;
        if (rect.x - rect_x as f32).abs() > 0.001
            || (rect.y - rect_y as f32).abs() > 0.001
            || (rect.width - texture.width as f32).abs() > 0.001
            || (rect.height - texture.height as f32).abs() > 0.001
        {
            return false;
        }

        let x0 = rect_x
            .max(self.viewport.x as i32)
            .max(0) as u32;
        let y0 = rect_y
            .max(self.viewport.y as i32)
            .max(0) as u32;
        let x1 = (rect_x + texture.width as i32)
            .min((self.viewport.x + self.viewport.width) as i32)
            .min(self.width as i32)
            .max(0) as u32;
        let y1 = (rect_y + texture.height as i32)
            .min((self.viewport.y + self.viewport.height) as i32)
        ...
        let src_x0 = (x0 as i32 - rect_x) as u32;
        let src_y0 = (y0 as i32 - rect_y) as u32;

        for row in 0..(y1 - y0) {
            let src_base = ((src_y0 + row) * texture.width + src_x0) as usize * 2;
            let dst_base = ((y0 + row) * self.width + x0) as usize;
            for col in 0..(x1 - x0) as usize {
                let src = src_base + col * 2;
                let dst = dst_base + col;
                if src + 1 < texture.data.len() && dst < self.framebuffer.len() {
                    self.framebuffer[dst] = rgb565_to_u32(texture.data[src], texture.data[src + 1]);
                }
            }
        }
    ...
    fn blit_a8_nearest(&mut self, rect: Rect, texture: &Texture, tint: Color) -> bool {
        if tint.a == 0 || rect.width <= 0.0 || rect.height <= 0.0 {
            return true;
        }

        let x0 = rect.x.max(self.viewport.x).max(0.0) as u32;
        let y0 = rect.y.max(self.viewport.y).max(0.0) as u32;
        let x1 = (rect.x + rect.width)
            .min(self.viewport.x + self.viewport.width)
            .min(self.width as f32)
            .max(0.0) as u32;
        let y1 = (rect.y + rect.height)
            .min(self.viewport.y + self.viewport.height)
        ...
        let inv_w = texture.width as f32 / rect.width.max(1.0);
        let inv_h = texture.height as f32 / rect.height.max(1.0);

        for y in y0..y1 {
            let sy = ((y as f32 - rect.y) * inv_h)
                .clamp(0.0, texture.height as f32 - 0.001) as u32;
            for x in x0..x1 {
                let sx = ((x as f32 - rect.x) * inv_w)
                    .clamp(0.0, texture.width as f32 - 0.001) as u32;
                let src = (sy * texture.width + sx) as usize;
                if src >= texture.data.len() {
                    continue;
                }

                let alpha = (texture.data[src] as u16 * tint.a as u16 / 255) as u8;
                if alpha == 0 {
                    continue;
                }

                let color = Color::new(tint.r, tint.g, tint.b, alpha);
                let dst = (y * self.width + x) as usize;
                if alpha == 255 && dst < self.framebuffer.len() {
                    self.framebuffer[dst] = color.to_u32();
                } else {
                    self.blend_pixel_fast(dst, color);
                }
            }
        }
...
#[inline]
fn region_is_full(region: &TextureRegion) -> bool {
    region.u0.abs() <= 0.0001
        && region.v0.abs() <= 0.0001
        && (region.u1 - 1.0).abs() <= 0.0001
        && (region.v1 - 1.0).abs() <= 0.0001
}

#[inline]
fn rgb565_to_u32(lo: u8, hi: u8) -> u32 {
