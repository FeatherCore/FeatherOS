...
/// Visual effect used by the app preview switcher.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PreviewEffect {
    /// Basic stacked cards. This is the baseline preview mode.
    Card,
    /// Project running surfaces onto a rotating football.
    Soccer,
}
...
impl PreviewEffect {
    ...
    pub const fn next(self) -> Self {
        match self {
            Self::Card => Self::Soccer,
            Self::Soccer => Self::Card,
        }
    }
}

impl Default for PreviewEffect {
    fn default() -> Self { Self::Card }
}
...
/// How Wing should launch an application entry.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum WingAppLaunchKind {
    /// A system UI surface hosted in the Wing shell loop.
    BuiltInSurface { surface_id: SurfaceId },
    /// A NuttX builtin command that should be launched as a separate task.
    NuttXBuiltin { surface_id: SurfaceId, command: &'static str },
}

impl WingAppLaunchKind {
    pub const fn surface_id(&self) -> SurfaceId {
        match *self {
            Self::BuiltInSurface { surface_id } => surface_id,
            Self::NuttXBuiltin { surface_id, .. } => surface_id,
        }
    }
}

/// Application entry shown by Wing launcher icons.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct WingAppEntry {
    pub id: u32,
    pub label: &'static str,
    pub preview_title: &'static str,
    pub icon_hint: &'static str,
    pub launch: WingAppLaunchKind,
    pub state: SurfaceState,
    pub pid: i32,
}

impl WingAppEntry {
    pub const fn built_in_surface(
        id: u32,
        ...
        Self {
            ...
            preview_title,
            icon_hint,
            launch: WingAppLaunchKind::BuiltInSurface { surface_id },
            state: SurfaceState::Background,
            pid: -1,
        }
    }

    pub const fn nuttx_builtin(
        id: u32,
        surface_id: SurfaceId,
        command: &'static str,
        label: &'static str,
        ...
        Self {
            ...
            preview_title,
            icon_hint,
            launch: WingAppLaunchKind::NuttXBuiltin { surface_id, command },
...
pub struct ShellContent {
    /// Launcher/application entries known to Wing
    pub apps: Vec<WingAppEntry>,
    /// Maximum number of launcher apps supported
    pub max_apps: usize,
    ...
    /// App switcher preview visual effect
        ...
        Self {
            ...
            surfaces: Vec::from([
                ...
                ShellSurfaceEntry::new(2, "Settings", "Settings App")
                    .with_state(SurfaceState::Running)
                    .with_icon("gear"),
                ShellSurfaceEntry::new(3, "Shell", "NuttX Shell")
                    .with_state(SurfaceState::Closed)
                    .with_icon("terminal"),
            ]),
            max_surfaces: 8,
            ...
            preview_effect: PreviewEffect::default(),
...
impl ShellContent {
    /// Add a launcher app entry.
    pub fn add_app(&mut self, app: WingAppEntry) -> bool {
    ...
    /// Get a launcher app by ID.
    pub fn get_app(&self, app_id: u32) -> Option<&WingAppEntry> {
        self.apps.iter().find(|app| app.id == app_id)
    }

    /// Get the launcher app that owns a shell surface.
    pub fn get_app_for_surface(&self, surface_id: SurfaceId) -> Option<&WingAppEntry> {
        self.apps.iter().find(|app| app.launch.surface_id() == surface_id)
    }

    /// Update launcher app runtime state.
    pub fn update_app_state(&mut self, app_id: u32, state: SurfaceState) -> bool {
        if let Some(app) = self.apps.iter_mut().find(|app| app.id == app_id) {
            app.state = state;
            true
        } else {
            false
    ...
    /// Store the task id associated with a launcher app.
    pub fn update_app_pid(&mut self, app_id: u32, pid: i32) -> bool {
        if let Some(app) = self.apps.iter_mut().find(|app| app.id == app_id) {
            app.pid = pid;
            true
        } else {
    ...
    /// Cycle app preview effect.
    pub fn cycle_preview_effect(&mut self) {
        self.preview_effect = self.preview_effect.next();
    }
}
