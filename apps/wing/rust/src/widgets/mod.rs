...
pub enum WidgetKind {
    ...
    Arc { value: u8 },
    Spinner,
    TextArea { text: String, placeholder: &'static str, cursor: usize, scroll_row: usize, selection_anchor: Option<usize> },
    List { title: &'static str, items: &'static [&'static str], selected: usize, scroll_row: usize },
    ...
    Dropdown { options: &'static [&'static str], selected: usize, expanded: bool, scroll_row: usize },
    Roller { options: &'static [&'static str], selected: usize },
    SpinBox { value: String },
...
impl WidgetTreeNode {
    ...
    pub fn find_focused_text_widget_id(&self) -> Option<WidgetId> {
        if self.widget.interaction.focused && matches!(self.widget.kind, WidgetKind::TextArea { .. }) {
            return Some(self.widget.id);
        }

        for child in &self.children {
            if let Some(widget_id) = child.find_focused_text_widget_id() {
...
impl WidgetNode {
    ...
    pub fn render(&self, origin: Vec2) -> Vec<RenderCommand> {
        ...
        match &self.kind {
            ...
            WidgetKind::TextArea { text, placeholder, cursor, scroll_row, selection_anchor } => {
                ...
                if !text.is_empty() {
                    for selection_rect in selection_rects(
                        ...
                        *scroll_row,
                        ...
                        commands.push(RenderCommand::draw_rect_rounded(selection_rect, self.accent_fill_color(), 3.0));
                    }
                }
                ...
                draw_vertical_scrollbar(
                    ...
                    rect,
                    *scroll_row,
                    wrapped_line_count(text.as_str(), rect.width - 20.0, 12.0),
                    visible_line_capacity(rect.height - 16.0, 12.0),
                    self.style.border,
                    self.accent_fill_color(),
                );
            }
            WidgetKind::List { title, items, selected, scroll_row } => {
                commands.push(RenderCommand::draw_rect_rounded(rect, self.fill_color(), self.style.radius));
                commands.push(RenderCommand::draw_text(Vec2::new(rect.x + 10.0, rect.y + 14.0), title, self.style.text, 12.0));
                let visible_rows = visible_list_rows(rect.height);
                for (slot, index) in (*scroll_row..(*scroll_row + visible_rows).min(items.len())).enumerate() {
                    let item = items[index];
                    let row = Rect::new(rect.x + 8.0, rect.y + 22.0 + slot as f32 * 22.0, rect.width - 24.0, 18.0);
                    if *selected == index {
            ...
            WidgetKind::Dropdown { options, selected, expanded, scroll_row } => {
                ...
                if *expanded {
                    let visible_rows = visible_dropdown_rows(options.len());
                    let popup_height = visible_rows as f32 * 18.0 + 8.0;
                    ...
                    for (slot, index) in (*scroll_row..(*scroll_row + visible_rows).min(options.len())).enumerate() {
                        let option = options[index];
                        let item_rect = Rect::new(popup.x + 4.0, popup.y + 4.0 + slot as f32 * 18.0, popup.width - 16.0, 16.0);
                        if *selected == index {
        ...
        match &self.kind {
            WidgetKind::Dropdown { options, expanded, .. } if *expanded => {
                let popup = Rect::new(
                    ...
                    self.rect.width,
                    visible_dropdown_rows(options.len()) as f32 * 18.0 + 8.0,
                );
    ...
    pub fn press(&mut self, point: Vec2) -> WidgetResponse {
        ...
        if let WidgetKind::TextArea { cursor, text, scroll_row, selection_anchor, .. } = &mut self.kind {
    ...
    pub fn release(&mut self, point: Vec2) -> WidgetResponse {
        ...
        if matches!(self.kind, WidgetKind::TextArea { .. }) {
            return WidgetResponse {
                consumed: true,
                event: Some(WidgetEvent {
                    widget_id: self.id,
    ...
    pub fn drag(&mut self, point: Vec2) -> WidgetResponse {
        ...
        match &mut self.kind {
            ...
            WidgetKind::TextArea { text, cursor, scroll_row, selection_anchor, .. } => {
                let local_x = (point.x - self.rect.x - 10.0).max(0.0);
                let local_y = (point.y - self.rect.y - 8.0).max(0.0) + *scroll_row as f32 * text_line_height(12.0);
                let next_cursor = cursor_from_point(text.as_str(), local_x, local_y, self.rect.width - 20.0, 12.0);
                if next_cursor != *cursor {
                    *cursor = next_cursor;
                    if selection_anchor.is_none() {
                        *selection_anchor = Some(next_cursor);
                    }
                    sync_textarea_scroll(text.as_str(), cursor, scroll_row, self.rect.width - 20.0, self.rect.height - 16.0, 12.0);
                    return WidgetResponse {
                        consumed: true,
    ...
    fn activate(&mut self, point: Vec2, terminal_event: WidgetEventKind) -> WidgetResponse {
        ...
        match &mut self.kind {
            ...
            WidgetKind::TextArea { cursor, text, scroll_row, selection_anchor, .. } => {
                self.interaction.focused = true;
                let local_x = (point.x - self.rect.x - 10.0).max(0.0);
                let local_y = (point.y - self.rect.y - 8.0).max(0.0) + *scroll_row as f32 * text_line_height(12.0);
                *cursor = cursor_from_point(text.as_str(), local_x, local_y, self.rect.width - 20.0, 12.0);
            ...
            WidgetKind::Dropdown { options, selected, expanded, scroll_row } => {
                ...
                if *expanded && point.y >= popup_top {
                    let item_index = *scroll_row + ((point.y - popup_top - 4.0) / 18.0) as usize;
                    ...
                    *expanded = !*expanded;
                    if *expanded {
                        *scroll_row = dropdown_scroll_for_selected(*selected, options.len(), *scroll_row);
                    }
                    WidgetResponse {
                        consumed: true,
                        event: Some(WidgetEvent { widget_id: self.id, kind: WidgetEventKind::ToggleChanged }),
            ...
            WidgetKind::List { items, selected, scroll_row, .. } => {
                let first_row_y = self.rect.y + 22.0;
                if point.y >= first_row_y {
                    let row = *scroll_row + ((point.y - first_row_y) / 22.0) as usize;
                    if row < items.len() {
                        *selected = row;
                        *scroll_row = list_scroll_for_selected(*selected, items.len(), *scroll_row, visible_list_rows(self.rect.height));
                        return WidgetResponse {
                            consumed: true,
                            event: Some(WidgetEvent { widget_id: self.id, kind: WidgetEventKind::SelectionChanged }),
                            action: self.action,
    ...
    pub fn handle_text_edit(&mut self, command: TextEditCommand) -> bool {
        match &mut self.kind {
            WidgetKind::TextArea { text, cursor, scroll_row, selection_anchor, .. } if self.interaction.focused => {
                match command {
                    ...
                    TextEditCommand::Backspace => {
                        ...
                        } else if *cursor > 0 && *cursor <= text.len() {
                            let idx = *cursor - 1;
                            text.remove(idx);
                            *cursor = idx;
                        }
                    ...
                    TextEditCommand::Cancel => return false,
            ...
            WidgetKind::List { items, selected, scroll_row, .. } if self.interaction.focused => {
                if items.is_empty() {
                    return false;
                }
                match command {
            ...
            WidgetKind::Dropdown { options, selected, expanded, scroll_row } if self.interaction.focused => {
                if options.is_empty() {
                    return false;
                }
                match command {
    ...
    pub fn handle_scroll(&mut self, delta: i32) -> bool {
        match &mut self.kind {
            WidgetKind::TextArea { text, scroll_row, .. } => {
                let next = scroll_by_delta(*scroll_row, delta, wrapped_line_count(text.as_str(), self.rect.width - 20.0, 12.0), visible_line_capacity(self.rect.height - 16.0, 12.0));
                if next != *scroll_row {
                    *scroll_row = next;
                    return true;
                }
            ...
            WidgetKind::List { items, scroll_row, .. } => {
                let next = scroll_by_delta(*scroll_row, delta, items.len(), visible_list_rows(self.rect.height));
                if next != *scroll_row {
                    *scroll_row = next;
                    return true;
                }
            ...
            WidgetKind::Dropdown { options, expanded, scroll_row, .. } if *expanded => {
                let next = scroll_by_delta(*scroll_row, delta, options.len(), visible_dropdown_rows(options.len()));
                if next != *scroll_row {
                    *scroll_row = next;
                    return true;
                }
...
fn sync_textarea_scroll(text: &str, cursor: &usize, scroll_row: &mut usize, width: f32, height: f32, size: f32) {
    ...
    let visible_rows = visible_line_capacity(height, size);
...
fn visible_line_capacity(height: f32, size: f32) -> usize {
    ((height / text_line_height(size)) as usize).max(1)
}

fn wrapped_line_count(text: &str, width: f32, size: f32) -> usize {
    wrap_text_to_width(text, width, size).lines().count().max(1)
}

fn visible_list_rows(height: f32) -> usize {
    (((height - 26.0).max(22.0)) / 22.0) as usize
}

fn visible_dropdown_rows(total: usize) -> usize {
    total.min(5).max(1)
}

fn max_scroll_row(total_rows: usize, visible_rows: usize) -> usize {
    total_rows.saturating_sub(visible_rows.max(1))
}

fn scroll_by_delta(current: usize, delta: i32, total_rows: usize, visible_rows: usize) -> usize {
    let max_row = max_scroll_row(total_rows, visible_rows) as i32;
    (current as i32 - delta).clamp(0, max_row) as usize
}

fn list_scroll_for_selected(selected: usize, total_rows: usize, current: usize, visible_rows: usize) -> usize {
    if selected < current {
        selected
    } else if selected >= current + visible_rows {
        selected + 1 - visible_rows.min(total_rows.max(1))
    } else {
        current.min(max_scroll_row(total_rows, visible_rows))
    }
}

fn dropdown_scroll_for_selected(selected: usize, total_rows: usize, current: usize) -> usize {
    list_scroll_for_selected(selected, total_rows, current, visible_dropdown_rows(total_rows))
}
...
fn draw_vertical_scrollbar(
    ...
    if total_rows <= visible_rows || rect.height <= 16.0 {
        return;
    }

    let track = Rect::new(rect.right() - 8.0, rect.y + 4.0, 4.0, rect.height - 8.0);
    let ratio = (visible_rows as f32 / total_rows as f32).clamp(0.15, 1.0);
    let thumb_h = (track.height * ratio).max(10.0);
    let max_row = max_scroll_row(total_rows, visible_rows).max(1) as f32;
    let progress = scroll_row as f32 / max_row;
    let thumb_y = track.y + (track.height - thumb_h) * progress;

    commands.push(RenderCommand::draw_rect_rounded(track, track_color, 999.0));
    commands.push(RenderCommand::draw_rect_rounded(
...
fn delete_selected_text(text: &mut String, cursor: &mut usize, selection_anchor: &mut Option<usize>) -> bool {
    let Some((start, end)) = selected_range(*selection_anchor, *cursor) else {
        return false;
    };
...
fn selection_rects(
    ...
    for idx in start..end {
        ...
        let caret = caret_position(text, idx, rect.width - 20.0, size);
        let draw_y = rect.y + 8.0 + caret.y - scroll_row as f32 * line_height;
        if draw_y < rect.y + 4.0 || draw_y + line_height > rect.bottom() - 4.0 {
            continue;
        }
        rects.push(Rect::new(
            rect.x + 10.0 + caret.x,
...
pub fn default_widget_tree_for_app(app: &AppInfo) -> Vec<WidgetTreeNode> {
    match app.name {
        "Calculator" => vec![
            make_column_panel(
                app,
                Rect::new(16.0, 96.0, 210.0, 198.0),
                vec![
                    WidgetTreeNode::new(WidgetNode::new(WidgetKind::Label { text: "SpinBox" }, Rect::new(0.0, 0.0, 80.0, 18.0), app.icon_color)),
                    WidgetTreeNode::new(WidgetNode::new(WidgetKind::SpinBox { value: String::from("0") }, Rect::new(0.0, 0.0, 200.0, 24.0), app.icon_color)),
                    WidgetTreeNode::new(WidgetNode::new(WidgetKind::ButtonMatrix { labels: &["7", "8", "9", "/", "4", "5", "6", "*", "1", "2", "3", "-", "0", ".", "=", "+", "(", ")", "DEL", "C"], columns: 4, selected: None }, Rect::new(0.0, 0.0, 220.0, 136.0), app.icon_color)
        ...
        "Files" => vec![
            ...
            make_row_panel(
                ...
                vec![
                    WidgetTreeNode::new(WidgetNode::new(WidgetKind::List { title: "Recent", items: &["desktop", "notes", "capture"], selected: 1, scroll_row: 0 }, Rect::new(0.0, 0.0, 180.0, 96.0), app.icon_color)
        ...
        "Terminal" => vec![
            make_column_panel(
                ...
                vec![
                    WidgetTreeNode::new(WidgetNode::new(WidgetKind::TextArea { text: String::from("$ wing --status"), placeholder: "command", cursor: 14, scroll_row: 0, selection_anchor: None }, Rect::new(0.0, 0.0, 320.0, 88.0), app.icon_color)),
                ...
                vec![
                    make_column_panel(
                        ...
                        vec![
                            ...
                            WidgetTreeNode::new(WidgetNode::new(WidgetKind::Dropdown { options: &["Aurora", "Dusk", "System"], selected: 0, expanded: false, scroll_row: 0 }, Rect::new(0.0, 0.0, 160.0, 24.0), app.icon_color)
