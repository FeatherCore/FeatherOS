                      ...
                      uint32_t addr, FAR uint8_t *value)
{
  ...
  cmd = ESP32_SPI_READ_REG_CMD |
        ((addr & 0x1FFFF) << ESP32_SPI_HDR_ADDR_SHIFT);
                       ...
                       uint32_t addr, FAR uint8_t *data, size_t len)
{
  ...
  cmd = ESP32_SPI_READ_FIFO_CMD |
        ((addr & 0x1FFFF) << ESP32_SPI_HDR_ADDR_SHIFT) |
        ((len & 0x7F) << ESP32_SPI_HDR_LEN_SHIFT);
                              ...
                              FAR uint32_t *len)
{
  ...
  cmd = ESP32_SPI_READ_REG_CMD |
        ((ESP32_SPI_SLCHOST_PKT_LEN_REG & 0x1FFFF) << ESP32_SPI_HDR_ADDR_SHIFT);
...
int esp32_spi_initialize(FAR struct spi_dev_s *spi_dev, int cs_gpio, int irq_gpio)
{
  ...
  /* Enable interrupts */
  ret = esp32_spi_enable_interrupt(priv);
  if (ret < 0)
    {
      esp32_spierr("Failed to enable interrupts: %d\n", ret);
      nxsem_destroy(&priv->lock);
      return ret;
    }
